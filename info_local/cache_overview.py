...
if not NAME:
    print("Error: NAME not specified. Usage: python cache_overview.py <chain_name>")
    sys.exit(1)
...
try:
    ...
    for row in top10_authorizers_rows:
        auth = dict(row)
        # MySQL doesn't have historical_code_address field, skip this line
    ...
    end_time = time.time()
    print(f"Cached {cache_path} in {end_time - start_time:.2f} seconds")
